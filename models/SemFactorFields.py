...
class MLPMixer(torch.nn.Module):
    def __init__(self,
        ...
        self.pool = nn.MaxPool1d(self.sem_dim)
        # config add
        # self.scene_bounding_sphere = 30.0
        self.sdf_bounding_sphere = 30.0
        self.sphere_scale = 100
        self.sigmoid = 10  # sigmoid
    ...
    def get_outputs(self, x,is_train=False):
        x.requires_grad_(True)
        output = self.forward(x,is_train)
        sdf_raw = output[:, :self.sem_dim]
        ...
        feature_vectors = output[:, self.sem_dim:]
    ...
    def get_specific_outputs(self, x, idx):
        ...
        gradients = torch.autograd.grad(
            ...
            retain_graph=True,
            only_inputs=True)[0]
        return sdf, feature_vectors, gradients, semantic, output[:, :self.sem_dim]

    def get_sdf_vals(self, x):
        sdf = self.forward(x)[:, :self.sem_dim]
        ''' Clamping the SDF with the scene bounding sphere, so that all rays are eventually occluded '''
...
class SemFactorFields(torch.nn.Module):
    def __init__(self, cfg, device):
        super(SemFactorFields, self).__init__()
        ...
        self.sem_dim = self.cfg.model.sem_dim
        self.linear_mat = MLPMixer(in_dim, out_dim + self.sem_dim,sem_dim=self.sem_dim, num_layers=cfg.model.num_layers, hidden_dim=cfg.model.hidden_dim,
        ...
        if 'reconstruction' in cfg.defaults.mode:
            ...
            self.renderModule = MLPRender_Fea(inChanel=out_dim , num_layers=num_layers, hidden_dim=hidden_dim,
    ...
    def forward(self, rays_chunk, white_bg=True, is_train=False, ndc_ray=False, N_samples=-1):
        ...
        if ray_valid.any():
            ...
            sdf, feature_vectors, gradients, semantic, sdf_raw = self.linear_mat.get_outputs(feats, is_train=is_train)
            # feat = self.linear_mat(feats, is_train=is_train)
            sigma[ray_valid] = self.basis2density(sdf[...,0])
        ...
        if app_mask.any():
            valid_rgbs = self.renderModule(viewdirs[ray_valid_new], feature_vectors[app_mask])
